...
class AccessibilityRecommender(QObject):
    ...
    def _load_recommendation_prompt(self) -> str:
        """
        Загружает промт для рекомендателя из файла interface/data/recommendation_prompt.md.
        Если файл не найден — возвращает резервный промт.
        """
        prompt_path = os.path.join(PROJECT_ROOT, "data", "recommendation_prompt.md")
        try:
            with open(prompt_path, "r", encoding="utf-8") as f:
                prompt = f.read()
            BasicUtils.logger("AccessibilityRecommender", "INFO", f"Промт загружен из {prompt_path}")
            return prompt
        except FileNotFoundError:
            BasicUtils.logger("AccessibilityRecommender", "ERROR", f"Файл с промтом не найден: {prompt_path}")
            return self._get_reserve_prompt()
        except Exception as e:
            BasicUtils.logger("AccessibilityRecommender", "ERROR", f"Ошибка чтения промта: {e}")
            return self._get_reserve_prompt()

    def _get_reserve_prompt(self) -> str:
        """
        Резервный промт (на случай, если файл не найден).
        """
        return (
    ...
    def _build_prompts(self, dysfunctions: str):
        ...
        Системный промт загружается из файла interface/data/recommendation_prompt.md.
        Пользовательский промпт формируется динамически на основе нарушений пользователя.
        """
        system_prompt = self._load_recommendation_prompt()
        user_prompt = (
            ...
            "Используй промт выше для формирования ответа."
        )
