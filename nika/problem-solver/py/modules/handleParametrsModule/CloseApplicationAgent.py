...
class CloseApplicationAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("action_close_app")
    ...
    def on_event(self, _1: ScAddr, _2: ScAddr, action_addr: ScAddr) -> ScResult:
        ...
        if application == "браузер":
            ...
            subprocess.Popen(["pkill", "-f", "gnome-control-center"])

        elif application == "проводник":
            subprocess.Popen(["pkill", "-f", "nautilus"])
    ...
    def _set_unk_app_message(self, action_addr: ScAddr) -> None:
        ...
        error_message_link = generate_link('Данного приложения не существует в базе знаний или он не открыто')
        templ = ScTemplate()
        templ.quintuple(
            action_addr,
            sc_type.VAR_COMMON_ARC,
